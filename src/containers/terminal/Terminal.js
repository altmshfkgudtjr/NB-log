	...
	const do_command = (text) => {
		...
		if (text === '') {
			...
			if (text.split(' ').length === 1) {
				msgs.push({ type: 'text', content: `'sudo'은(는) 내부 또는 외부 명령, 실행할 수 있는 프로그램, 또는 배치 파일이 아닙니다.` });
			} else {
				msgs.push({ type: 'text', content: `'${cmd}'은(는) 내부 또는 외부 명령, 실행할 수 있는 프로그램, 또는 배치 파일이 아닙니다.` });
			}
